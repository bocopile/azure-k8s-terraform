...
az keyvault purge --name kv-k8s-<suffix>

# 방법 2: Recover (복구 후 재사용)
az keyvault recover --name kv-k8s-<suffix>
```
...
```bash
# 현재 설정 확인
az keyvault show --name kv-k8s-<suffix> --query "properties.enablePurgeProtection"
...
az group list --query "[?starts_with(name, 'rg-k8s')]" -o table

# MC_* 노드 리소스 그룹 확인 (AKS 삭제 시 자동 제거)
az group list --query "[?starts_with(name, 'MC_rg-k8s')]" -o table

# Soft-deleted Key Vault 확인
az keyvault list-deleted --query "[?starts_with(name, 'kv-k8s')]" -o table

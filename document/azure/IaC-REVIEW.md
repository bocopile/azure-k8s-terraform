...
| keyvault | Key Vault×1, RoleAssignment×1 | 2 |
| acr | ACR×1 | 1 |
| monitoring | LAW×1, MonitorWorkspace×1, AppInsights×1 | 3 |
| backup | BackupVault×1, BackupPolicy×1 | 2 |
| aks | RG×3, AKS×3, NodePool×9, Bastion×1, JumpVM×1, NIC×1, PIP×1 | ~20 |
| **합계** | | **~68 리소스** |
...
| **고정되지 않는 것** (현재 코드) | Jump VM NIC (Dynamic), AKS 노드 IP (동적) |
...
```hcl
# modules/aks/main.tf:195 — 현재 Dynamic (변경 필요)
ip_configuration {
  name                          = "ipconfig-jumpbox"
  subnet_id                     = var.jumpbox_subnet_id
  private_ip_address_allocation = "Dynamic"   # ← 재기동 시 IP 변경 가능
}
```
...
| 대상 | 방법 | 우선순위 |
|------|------|---------|
| Jump VM NIC | `Static` + 명시적 IP (`10.1.1.10`) | **High** — Bastion 접속 타겟 고정 |
...
```hcl
# modules/aks/main.tf — 수정 예시
ip_configuration {
  name                          = "ipconfig-jumpbox"
  subnet_id                     = var.jumpbox_subnet_id
  private_ip_address_allocation = "Static"
  private_ip_address            = var.jumpbox_private_ip  # 신규 variable 추가
}
```

```hcl
# modules/aks/variables.tf — 신규 추가
...
**② Backup Vault Soft Delete**

```hcl
# modules/backup/main.tf:14
soft_delete = "On"   # 삭제 후 데이터 보존
```

- BackupInstance가 존재하는 상태에서 Vault 삭제 시도 → 오류 발생
- **순서**: BackupInstance 먼저 삭제 → Vault 삭제
- `tofu destroy`는 이 순서를 state 의존성으로 처리하지만, BackupInstance는 Addon 단계(스크립트)에서 생성되므로 **state 밖에 존재**할 수 있음
...
| Backup Vault | BackupInstance IaC 관리로 state 포함 | High |
| 삭제 가이드 | `DESTROY.md` 작성 (순서/주의사항) | Medium |
| `tofu destroy` 전처리 | Pre-destroy script (K8s 리소스 정리) | Medium |

#### 개선 코드 (Backup Vault — soft delete 조건부)

```hcl
# modules/backup/main.tf — 개선 예시
resource "azurerm_data_protection_backup_vault" "vault" {
  # ...
  soft_delete = var.enable_soft_delete ? "On" : "Off"  # Demo는 Off 허용
}
```

```hcl
# modules/backup/variables.tf — 신규 추가
variable "enable_soft_delete" {
  description = "Enable soft delete on Backup Vault (false for dev/demo for easy cleanup)"
  type        = bool
  default     = false  # Demo 환경: 즉시 삭제 허용
}
```
...
| # | 파일 | 현재 상태 | 개선 내용 |
|---|------|---------|---------|
| H1 | `modules/aks/main.tf` | Jump VM NIC: `Dynamic` | `Static` + `10.1.1.10` 고정 |
| H2 | `modules/aks/variables.tf` | jumpbox_private_ip 변수 없음 | 신규 변수 추가 (`default = "10.1.1.10"`) |
| H3 | `modules/backup/main.tf` | `soft_delete = "On"` 하드코딩 | 변수화 (`enable_soft_delete`, Demo 기본값 `false`) |
| H4 | `modules/backup/variables.tf` | enable_soft_delete 변수 없음 | 신규 변수 추가 |
...
```
Phase 1 (즉시): H1 ~ H4 코드 수정
  └─ Jump VM IP 고정 (Static)
  └─ Backup Vault soft_delete 변수화
...
*이 문서는 구현 검토 결과를 반영하며, 코드 수정 전 합의 기반으로 활용됩니다.*

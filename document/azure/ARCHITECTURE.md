...
| **가용성 목표** | 베스트 에포트 (Free Tier, 데모 환경) — 운영 전환 시 Standard Tier (99.95% SLA) |
| **RTO** | 30분 (AKS 재생성 기준) |
| **RPO** | 24시간 (일일 백업 기준) |
...
| **리전/가용성** | Korea Central — AZ Zone 1/2/3, AKS Free Tier (데모) / Standard Tier (운영) |
| **컴퓨팅** | AKS Managed (v1.35), Spot VM, Node Auto-Provisioning (Karpenter) |
...
| **관찰성** | Azure Managed Prometheus (DCR) + Managed Grafana + Container Insights + Application Insights + OTel Collector + Cilium Hubble + Tetragon + Kiali v2.22 + VPA + Alert Rules + Diagnostic Settings + NSG Flow Logs + Activity Log→LAW |
...
| **관리자 접근** | Authorized IP Ranges (로컬 kubectl 직접 접근) — `enable_jumpbox=true` 시 Azure Bastion + Jump VM 전환 가능 |

### 1.4 제약 조건

- Ansible 미사용 (Shell Script로 대체)
- Helmfile 미사용 (Helm CLI 직접 사용)
- AKS Free Tier (데모 비용 절약, SLA 없음) — 운영 시 Standard Tier ($0.10/cluster/h, 99.95% SLA) 전환 필수
- AKS Istio Add-on: **asm-1-28** (v1.35 지원, EOL Aug 2026)
...
| **ADR-002** | Accepted | AKS Free Tier (데모) / Standard Tier (운영) + Zone 분산. 데모: `aks_sku_tier = "Free"` |
...
| **ADR-021** | Updated | **[현재]** Authorized IP Ranges (`enable_private_cluster=false`) — 로컬 공인 IP만 허용, kubectl 직접 접근. **[옵션]** `enable_jumpbox=true` 시 Azure Bastion + Jump VM + Private Cluster 전환 가능 |
...
| **C15** | AKS API Server 접근은 반드시 제어됨 — `enable_private_cluster=false` 시 Authorized IP, `true` 시 Private Cluster + Jump VM 경유 | ADR-021 |
...
| 클러스터 | 역할 | 클러스터 내 컴포넌트 |
|---------|------|-------------------|
| **mgmt** | 플랫폼 서비스 | Istio asm-1-28 (AKS Add-on), Kiali v2.22, cert-manager v1.19.x, Flux v2 |
...
> **AKS Tier**: Free (데모, `aks_sku_tier = "Free"`) — 운영 시 Standard ($0.10/cluster/h, 99.95% SLA)
> **리전**: Korea Central — Zone 1, 2, 3 분산 (Cross-AZ 트래픽 무료)

| 클러스터 | Node Pool | VM Size | 노드 수 | Priority | AZ | 비고 |
|---------|-----------|---------|--------|----------|----|------|
| mgmt | system | Standard_D2s_v4 | **3** | Regular | 1,2,3 | Zone당 1노드 |
| mgmt | ingress | Standard_D2s_v4 | **3** | Regular | 1,2,3 | Istio Ingress Gateway 전용 |
| mgmt | worker | Standard_D2s_v4 | 0+ | **Spot** | 1,2,3 | Kiali, cert-manager (Karpenter) |
| app1 | system | Standard_D2s_v4 | **3** | Regular | 1,2,3 | Zone당 1노드 |
| app1 | ingress | Standard_D2s_v4 | **3** | Regular | 1,2,3 | Istio Ingress Gateway 전용 |
| app1 | worker | Standard_D2s_v4 | 0+ | **Spot** | 1,2,3 | 앱 워크로드 (Karpenter) |
| app2 | system | Standard_D2s_v4 | **3** | Regular | 1,2,3 | Zone당 1노드 |
| app2 | worker | Standard_D2s_v4 | 0+ | **Spot** | 1,2,3 | 앱 워크로드 (Karpenter) |

> Standard_D2s_v4 = 2 vCPU / 8GB RAM (`vm_size_system` / `vm_size_ingress` 변수로 변경 가능)
...
|---------------|-----------|
| **rg-k8s-common** | VNet(×3), NSG(×3), Key Vault, ACR, DNS Zone, Azure Monitor Workspace, Log Analytics Workspace, Backup Vault |
| **rg-k8s-mgmt** | AKS mgmt, Managed Identity — `enable_jumpbox=true` 시: Azure Bastion + Jump VM (B2s, private IP) 추가 |
| **rg-k8s-app1** | AKS app1, Managed Identity |
| **rg-k8s-app2** | AKS app2, Managed Identity |
...
- CRD 버전: **v1.5.0** (수동 설치 — `00b-gateway-api.sh`, Istio GA 전환 대비 사전 배포)
...
| **AKS API Server** | **[현재]** Authorized IP Ranges (`enable_private_cluster=false`) — 공개 엔드포인트 + IP 필터링. **[옵션]** `enable_private_cluster=true` 시 Private Cluster + VNet 내부 전용 |
| **AKS Private DNS Zone** | `enable_private_cluster=true` 시 공유 DNS Zone 생성 + 3개 VNet 링크. 현재(`false`) 미생성 |

> **Authorized IP 방식**: API Server 공개 엔드포인트를 유지하되 허용 IP를 `api_server_authorized_ips`로 제한.
> 로컬 개발 환경에서 kubectl 직접 접근 가능 (Bastion/Jump VM 불필요).
> IP 변경 시 `terraform.tfvars`의 `api_server_authorized_ips` 업데이트 필요.
>
> **Private Cluster 전환**: `enable_private_cluster=true` + `enable_jumpbox=true` 설정 시
> Azure Bastion + Jump VM + Private DNS Zone이 함께 배포됨.

### 5.7 관리자 접근 아키텍처 (ADR-021)

**현재 구성: Authorized IP Ranges (`enable_jumpbox = false`)**

```
[관리자 PC (공인 IP: x.x.x.x)]
    │  kubectl / az CLI (직접)
    ▼
AKS API Server (공개 엔드포인트 + api_server_authorized_ips 필터)
```

| 항목 | 설정 |
|-----|------|
| **접근 방식** | Authorized IP Ranges (`enable_private_cluster = false`) |
| **허용 IP** | `api_server_authorized_ips = ["x.x.x.x/32"]` — 로컬 공인 IP |
| **Jump VM** | 미배포 (`enable_jumpbox = false`) |
| **Azure Bastion** | 미배포 (`enable_jumpbox = false`) |
| **비용** | 없음 (Bastion ~$140/월 절감) |
...
api_server_authorized_ips = ["x.x.x.x/32"]  # curl -s ifconfig.me

# modules/aks/main.tf
private_cluster_enabled = var.enable_private_cluster  # false
...
**[옵션] Private Cluster + Jumpbox 전환 (`enable_jumpbox = true`)**:

```
[관리자 PC]
    │  HTTPS (443)
    ▼
Azure Bastion (PaaS, AzureBastionSubnet 10.1.100.0/26)
    │  SSH (브라우저 기반)
    ▼
Jump VM (Standard_B2s, private IP 10.1.4.10, mgmt VNet)
    │  kubectl / az CLI
    ▼
...
```hcl
# terraform.tfvars (Private Cluster 전환 시)
enable_private_cluster = true
enable_jumpbox         = true
```

> **IP 변경 주의**: 동적 공인 IP 사용 시 IP 변경 때마다 `api_server_authorized_ips` 업데이트 + `tofu apply` 필요.
> CI/CD 파이프라인에서 kubectl 접근 시 에이전트 공인 IP도 목록에 추가해야 함.

### 5.8 DNS 자동화

| 항목 | 설정 |
|-----|------|
| **Azure DNS Zone** | rg-k8s-common에 생성 |
    ...
    solvers:
      - dns01:
          azureDNS:
            ...
            resourceGroupName: rg-k8s-common
...
| **Backup Vault** | `bv-k8s` (rg-k8s-common, **ZoneRedundant**) |
...
|---------|------------|--------------|--------|
| Standard_D2s_v4 (2vCPU/8GB) | ~$0.096 | ~$0.019 | ~80% |
...
- AKS Free Tier (데모): $0/cluster — 운영 시 Standard: $0.10/cluster/h × 3 클러스터
- Regular D2s_v4: ~$0.096/h (Korea Central 기준)
- Spot D2s_v4: ~$0.019/h (~80% 절감, 시장 변동 가능)
...
| 리소스 | 수량 | 단가 | 2h 비용 |
|-------|------|------|--------|
| AKS Control Plane (Free/Standard) | 3 | $0.10/h | $0.60 |
| System Node (Regular D2s_v4) | 9 | ~$0.096/h | $1.73 |
| Ingress Node (Regular D2s_v4) | 6 | ~$0.096/h | $1.15 |
| Worker Node (Spot D2s_v4) | 3 | ~$0.019/h | $0.11 |
...
| 리소스 | 수량 | 단가 | 24h 비용 |
|-------|------|------|---------|
| AKS Control Plane (Free/Standard) | 3 | $0.10/h | $7.20 |
| System Node (Regular D2s_v4) | 9 | ~$0.096/h | $20.74 |
| Ingress Node (Regular D2s_v4) | 6 | ~$0.096/h | $13.82 |
| Worker Node (Spot D2s_v4) | 3 | ~$0.019/h | $1.37 |
...
| System Node (Regular D2s_v4 × 9) | 9 | ~$138 |
| Ingress Node (Regular D2s_v4 × 6) | 6 | ~$92 |
| Worker Node (Spot D2s_v4 × 3) | 3 | ~$9 |
...
| 리소스 | 최소 필요 쿼터 | 확인 명령 |
|-------|-------------|----------|
| Standard Dv4 Regular vCPUs | **30** (system 9 + ingress 6 × 2vCPU) | `az vm list-usage -l koreacentral -o table` |
| Standard Dv4 Spot vCPUs | **6** (worker 3 × 2vCPU, 최소) | 동일 |
| Public IP Addresses | **3** (AKS LB ×3, Bastion 미배포) | 동일 |
| Load Balancers | 3 | 동일 |

> `enable_jumpbox = false` (기본): Bastion용 Public IP 불필요. `true` 설정 시 Public IP +1개 필요.
...
```hcl
# AKS 클러스터 (Free Tier 데모 / Standard Tier 운영)
resource "azurerm_kubernetes_cluster" "aks" {
  sku_tier           = var.aks_sku_tier   # "Free" (데모) / "Standard" (운영)
  kubernetes_version = "1.35"
  location           = local.location
...
| 09 | *(skip)* | — | Backup Extension은 **Terraform에서 관리** (`tofu apply`) | — |
...
```bash
# Step 1: Azure Portal → rg-k8s-mgmt → bastion-k8s → Connect
